ui_dialoguebox_create(xx+camerachangex, yy, width, height);

if (asset_get_type(face) == asset_sprite) {
	draw_sprite(face,faceid,xx+camerachangex+13,yy+14)
...
if (instance_exists(t)) {
	if (on_choice) {
		...
		for(var i = 0; i < array_length(choices); i++) {
			...
			if (ch_index == i) {
				draw_sprite_ext(spr_ui_soul,0,pos.x-sprite_get_width(spr_ui_soul)-4+camerachangex,pos.y+string_height(name)/3-2,1,1,0,c_red,draw_get_alpha)
			}
			draw_text(pos.x+camerachangex,pos.y,name)
		}
		draw_set_font(-1)
	}
}
